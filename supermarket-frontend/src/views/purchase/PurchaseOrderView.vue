<template>
  <div class="purchase-order-container">
    <el-card>
      ...
      <el-form :inline="true" :model="queryParams" style="margin-bottom: 15px">
...
const handleSubmit = async () => {
  ...
  const hasEmptyItem = orderForm.items.some(item => !item.productId || item.quantity <= 0)
  if (hasEmptyItem) {
...
const handleViewDetail = (row) => {
  ElMessage.info(`查看采购订单详情: ${row.billNo}`)
}
...
const handleVoid = async (row) => {
  try {
    await ElMessageBox.confirm('确认作废该采购订单吗？', '提示', {
      type: 'warning'
    })
